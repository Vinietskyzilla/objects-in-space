...
public class Game {
	...
	private /*synchronized*/ void spawnSpaceObjs() {
		//synchronized (astros) {
			astros.add(new Fighter(100, 100, getNextShipID() + level * 2));
			astros.add(new Fighter(-100, -100, getNextShipID() + level));
			astros.add(new Fighter(-200, 200, getNextShipID() + level));
		...
		synchronized (astros) {
			for(int i = 0; i < astroSize; i++) {
				...
				if(s instanceof Weapon) {
					((Weapon) s).distanceCovered += Math.sqrt(s.dx*s.dx + s.dy*s.dy);
					if(((Weapon) s).distanceCovered >= ((Weapon) s).range) {
						astros.remove(s);
			...
			synchronized (astros) {
				...
				for(int i = 0; i < astros.size(); i++) {
					...
					if(s instanceof Weapon) {
						...
						g2d.fillOval((int) (s.x + panelWidth / 2), (int) (-(s.y) + panelHeight / 2), 3, 3);
					}
					else {
